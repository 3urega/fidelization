...
import { ListCustomerPromotionSummaries } from "../../../../contexts/loyalty/promotions/application/list/ListCustomerPromotionSummaries";
...
export async function GET(request: Request): Promise<Response> {
	...
	try {
		const [stampProgress, rewards, promotions] = await Promise.all([
			...
			container.get(ListCustomerPromotionSummaries).execute({
				tenantId: auth.session.tenantId,
				customerId: auth.customer.id,
			}),
		]);
		...
		return NextResponse.json({
			...
			promotions: promotions.map((promotion) => customerPromotionSummaryToJson(promotion)),
