...
import { EnrollCustomerInRoulette } from "../../../../../../../../contexts/loyalty/games/application/participation/EnrollCustomerInRoulette";
...
import { rouletteEnrollResultToJson } from "../../../../../../../../lib/loyalty/roulettePublicStateJson";
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
	...
	try {
		...
		if (!detail.customer) {
			return NextResponse.json(
				{ error: { description: "Join this establishment before enrolling in the roulette" } },
				{ status: 403 },
			);
		}

		const result = await container.get(EnrollCustomerInRoulette).execute({
			tenantId: detail.tenant.id,
			customerId: detail.customer.id,
		});

		return NextResponse.json(rouletteEnrollResultToJson(result));
	} catch (error) {
