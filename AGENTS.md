...
npm run verify:platform-app-dashboard-use-case  # issue #40/#41 — ListUserRelationships (domain stub)
...
- **Platform app register business (#40):** `/u/register/business` (auth gate) → `/u/register/business/tenant`; `POST /api/user/businesses` con sesión `kind: user`; «Mis negocios» en `/u/home`. `verify:platform-app-register-business`, `verify:platform-app-dashboard-use-case`.
...
| Platform mobile app Phase G (#38–#45) | `verify:platform-app-auth-use-case` (#38), `verify:platform-app-public-home` (#39), `verify:platform-app-register-business` (#40) + `docs/domain/customer-platform-app.md` |
