...
npm run verify:roulette-scan-eligibility-use-case # Phase V6 #113 — IssueRouletteSpinEligibility + spin gate (domain stub; legacy regression)
npm run verify:roulette-scan-eligibility # Phase V6 #113 — staff scan → eligibility → spin E2E (dev + DATABASE_URL; config v1 legacy)
...
npm run verify:roulette-participation-flow-e2e # Phase X5 #122 — enroll → authorize → spin → premio + quota_exhausted E2E (dev + DATABASE_URL)
...
- **Owner gamification library (#81):** owner en `/settings/games` — lista juegos globales según plan (Premium + `gamification`); ruleta configurable v2 en `/settings/games/ruleta` (cuota periodo, min €, `staff_explicit`); cliente **activa ruleta en app** (`POST …/ruleta/enroll`) y gira en `/home/establishments/[slug]/ruleta` tras **autorización staff** (`roulette_authorize` + importe en `/scan`); legacy `after_staff_scan` sigue soportado (dual mode). Staff canjea premios físicos en `/scan`. Assets en `public/assets/ruleta/`. Superadmin CRUD en `/platform/games`. Verifies: `verify:platform-admin-games*`, `verify:tenant-games-list`, `verify:roulette-owner-config`, `verify:roulette-participation-flow-e2e`, `verify:roulette-staff-authorize*`, `verify:roulette-client-participation*`, `verify:roulette-spin*` (legacy v1), `verify:roulette-scan-eligibility*` (legacy), `verify:roulette-staff-redeem*`.
...
- **Ruleta v2 — flujo operativo caja + app (Phase X #118–#122):** (1) Cliente en detalle del local → activa ruleta (enroll). (2) En caja: staff en `/scan` elige **Autorizar giro de ruleta**, introduce importe €, escanea QR → `roulette_auth_granted` o rechazo tipado. (3) Cliente gira en app; premio puntos/sello/físico. Visita/sello es acción separada (tarjeta/promo). Demo `cafe-demo` usa config v2 (`DEMO_ROULETTE_CONFIG`). Spec: [`docs/domain/roulette-game.md`](docs/domain/roulette-game.md), [`docs/domain/staff-scan-flow.md`](docs/domain/staff-scan-flow.md). Verifies: `verify:roulette-participation-flow-e2e`, `verify:roulette-staff-authorize*`, `verify:roulette-client-participation*`.
- **Staff scan (Phase M #65–#70):** owner/empleado en `/scan` elige **tarjeta, promo o autorizar ruleta (v2)** → `GET /api/loyalty/scan/targets` + `POST /api/loyalty/scan` → `{ customer, outcomes[] }`. Config v2: ruleta vía `targetType: roulette_authorize` + `purchaseAmountEuros` (sin mezclar con sello). Legacy: `after_staff_scan` → outcome `roulette_spin_granted` al registrar visita. Canje premio físico: sección colapsada en `/scan`. `GET /api/loyalty/games/ruleta/scan-context` → `{ unlockEnabled, authorizeEnabled, minPurchaseEuros }`. Verifies: `verify:staff-scan-*`, `verify:staff-scan-roulette-ux`, `verify:roulette-participation-flow-e2e`, `verify:customer-scan`, `verify:platform-app-global-qr-scan*`, etc. Spec: [`docs/domain/staff-scan-flow.md`](docs/domain/staff-scan-flow.md).
