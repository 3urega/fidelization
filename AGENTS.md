...
- **Superadmin dashboard (issue #9):** en `/platform/tenants` — lista de negocios y activar/suspender. Home KPI en `/platform` (`GET /api/platform/dashboard`). `verify:platform-tenants`, `verify:platform-admin-dashboard*`.
...
| Superadmin dashboard (issue #9, #71) | `docs/superadmin.md` + `npm run verify:platform-tenants` + `verify:platform-admin-dashboard*` |
