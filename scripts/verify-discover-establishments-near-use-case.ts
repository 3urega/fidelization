...
class NearStubTenantRepository extends TenantRepository {
	...
	async listDiscoverableActive(params: ListDiscoverableEstablishmentsParams) {
		...
		if (params.near) {
			...
			const withDistance = this.activeTenants
			...
			const slice = withDistance.slice(skip, skip + params.limit + 1);
...
async function main(): Promise<void> {
	...
	if (nearResults.establishments.length !== 2) {
		console.error("❌ near mode should return 2 geocoded active tenants", nearResults);
		process.exit(1);
	}
	...
	if (
		...
		console.error("❌ near mode should order by distance", nearResults);
		process.exit(1);
	}
	...
	if (
		...
		console.error("❌ near mode should include distanceKm", nearResults);
		process.exit(1);
	}

	if (nearResults.establishments.some((row) => row.slug === "no-coords-cafe")) {
		console.error("❌ tenants without coords must be excluded in near mode");
		process.exit(1);
	}

	console.log("✅ near mode orders by distance with distanceKm");
	...
	if (
		legacy.establishments.length !== 3 ||
		legacy.establishments[0]?.slug !== "near-alpha" ||
