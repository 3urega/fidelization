...
- **Superadmin dashboard (issue #9):** en `/platform/tenants` — lista, detalle, impersonar owner; home KPI en `/platform`. `verify:platform-tenants`, `verify:platform-admin-dashboard*`, `verify:platform-admin-tenant-detail*`, `verify:platform-admin-impersonate*`.
...
| Superadmin dashboard (issue #9, #71–#73) | `docs/superadmin.md` + `verify:platform-tenants` + `verify:platform-admin-dashboard*` + `verify:platform-admin-tenant-detail*` + `verify:platform-admin-impersonate*` |
