...
import { ExecuteRouletteSpin } from "../../../../../../../../contexts/loyalty/games/application/spin/ExecuteRouletteSpin";
...
import { rouletteSpinResultToJson } from "../../../../../../../../lib/loyalty/roulettePublicStateJson";
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
	...
	try {
		...
		if (!detail.customer) {
			return NextResponse.json(
				{ error: { description: "Join this establishment before spinning the roulette" } },
				{ status: 403 },
			);
		}

		const result = await container.get(ExecuteRouletteSpin).execute({
