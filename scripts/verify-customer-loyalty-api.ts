...
async function main(): Promise<void> {
	...
	const apexRegister = await fetch(`${apexBaseUrl}/api/loyalty/customers/register`, {
		method: "POST",
	...
	const me = await fetch(`${apexBaseUrl}/api/loyalty/me`, {
	...
	const staffMe = await fetch(`${apexBaseUrl}/api/loyalty/me`, {
	...
	const qrAuth = await fetch(`${apexBaseUrl}/api/loyalty/auth/qr`, {
		method: "POST",
		headers: tenantHeaders({ "Content-Type": "application/json" }),
