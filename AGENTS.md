...
- **Superadmin dashboard (issue #9):** en `/platform/tenants` — lista de negocios y activar/suspender; detalle en `/platform/tenants/[tenantId]`. Home KPI en `/platform` (`GET /api/platform/dashboard`). `verify:platform-tenants`, `verify:platform-admin-dashboard*`, `verify:platform-admin-tenant-detail*`.
...
| Superadmin dashboard (issue #9, #71, #72) | `docs/superadmin.md` + `verify:platform-tenants` + `verify:platform-admin-dashboard*` + `verify:platform-admin-tenant-detail*` |
