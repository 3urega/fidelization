...
export function PlatformUserRegisterForm({
	...
	async function submit(e: React.FormEvent): Promise<void> {
		...
		try {
			const response = await platformFetch("/api/auth/register/user", {
				method: "POST",
				...
				setError(
					mapRegisterError(
						data?.error?.description ??
							platformApiFallbackMessage(response.status, "Error al registrarse"),
						data?.error?.type,
					),
				);
			...
			setError("Error de red al registrarse. Comprueba tu conexión.");
		} finally {
			setLoading(false);
		}
	}
	...
	return (
		<div className="flex flex-col gap-4">
			<PlatformGoogleSignInButton redirectTo={redirectTo} text="signup_with" />
			<PlatformAuthDivider />
